...
void AudioEffectProcessUsbPhone(AudioCoreContext *pAudioCore)
{
	...
	if(mic_in && pcm_in && pcm_out && dac_out)
	{
        ...
        //本地MIC采样做降噪处理
		#if CFG_AUDIO_EFFECT_MIC_NOISE_SUPPRESSOR_EN
		//AudioEffectExpanderApply(&gCtrlVars.mic_expander_unit, mic_in, mic_in, n);
		#endif

		AudioCoreAppSourceVolSet(MIC_SOURCE_NUM, mic_in, n, 2);

		#if CFG_AUDIO_EFFECT_MIC_BYPASS_EQ_EN
		//AudioEffectEQApply(&gCtrlVars.mic_bypass_eq_unit, mic_in, mic_in, n, 2);
		#endif
...
//			pcm_out[s*2] = d_pcm_tmp[s];//输出时做的立体声转换
//			pcm_out[s*2+1] = d_pcm_tmp[s];
//		}
//	    #endif
	...
	else if((dac_out)&&(remind_in))
	{
		...
		if(remind_in)
		{
			...
			for(s = 0; s < n; s++)
			{
				dac_out[2*s + 0] = remind_in[2*s + 0];
				dac_out[2*s + 1] = remind_in[2*s + 1];
			}
		}
	#endif
	...
	if(dac_out)
	{
		if(i2s0_out)
		{
			for(s = 0; s < n; s++)
			{
				i2s0_out[2*s + 0] = dac_out[2*s + 0];
				i2s0_out[2*s + 1] = dac_out[2*s + 1];
			}
		}
		if(i2s1_out)
		{
			for(s = 0; s < n; s++)
			{
				i2s1_out[2*s + 0] = dac_out[2*s + 0];
				i2s1_out[2*s + 1] = dac_out[2*s + 1];
			}
		}
	}
	...
	for(s = 0; s < n; s++)
	{
		...
		//对比AEC效果
//		pcm_out[2*s] = pcm_out[n+s];
//		pcm_out[2*s + 1] = d_pcm_tmp[s];

//		//Delay调试
//		pcm_out[2*s + 0] = d_pcm_tmp[s];
//		pcm_out[2*s + 1] = u_pcm_tmp[s];
	}
}
#endif
#endif
