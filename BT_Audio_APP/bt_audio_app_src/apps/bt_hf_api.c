...
{
	if(Audio_AECRingDataSpaceLenGet(&BtHfCt->AecContext) > Len)
	{
		Audio_AECRingDataSet(&BtHfCt->AecContext, PcmIn, Len);
	}
}

int16_t * btHfRingDataGet(uint32_t Len)
{
	return (int16_t *)Audio_AecInBuf(&BtHfCt->AecContext, Len);
}
