...
typedef struct _HdmiInPlayContext
{
	...
	int16_t 			pcmRemain[PCM_REMAIN_SMAPLES * 4];//���ڻ���ײ����������128samples������
...
static void HdmiARCScan(void)
{
	...
	while(cnt--)
	{
		...
		if(hdmiInPlayCt->pcmRemainLen > 0)
		{
			...
			memcpy(&pcmBuf[hdmiInPlayCt->pcmRemainLen * 2], &pcmBuf[MAX_FRAME_SAMPLES * 2 + PCM_REMAIN_SMAPLES * 2], pcm_len);
		...
		if((pcm_len / 4) > MAX_FRAME_SAMPLES)
		{
			...
			if(hdmiInPlayCt->pcmRemainLen > PCM_REMAIN_SMAPLES)
			{
				hdmiInPlayCt->pcmRemainLen = PCM_REMAIN_SMAPLES;
			}
			memcpy(hdmiInPlayCt->pcmRemain, &pcmBuf[MAX_FRAME_SAMPLES * 2], hdmiInPlayCt->pcmRemainLen * 4);
