...
//AudioCore sink2 config -- 用于HFP SCO发送数据
	...
	//AEC
	Audio_AECInit(&BtHfCt->AecContext);
...
/*#ifdef CFG_FUNC_REMIND_SOUND_EN
			...
			case MSG_TASK_START:
				...
				else
				{
					...
					Audio_AECEffectInit(BT_HFP_AEC_ECHO_LEVEL, BT_HFP_AEC_NOISE_LEVEL);
			...
			case MSG_TASK_PAUSE:
				...
				//	BtHfCt->btHfScoSendStart = 0;
					Audio_AECReset(&BtHfCt->AecContext, DEFAULT_DELAY_BLK);
				}
				break;
...
//				break;
		...
		{
			if(flagVoiceDelayExitBtHf++ >= 1000)
			{
...
//注意此处，如果在TaskStateCreating发起stop，它尚未init，暂时不会出错
	...
	//AEC
	Audio_AECDeinit(&BtHfCt->AecContext);
	//AudioEffectsDeInit_HFP();
	
#if ((CFG_AUDIO_EFFECT_MIC_PITCH_SHIFTER_EN)&&defined(BT_HFP_MIC_PITCH_SHIFTER_FUNC))
	//renew PitchShifter channel 
...
void ExitBtHfMode(void)
{
	...
	if(BtHfCt)
	{
		if(flagVoiceDelayExitBtHf && (flagVoiceDelayExitBtHf<=1000))
		{
			return;
		}
		flagVoiceDelayExitBtHf = 0;		
