...
//	uint16_t			*SourceBuf_Aec;
	AudioAECContext		AecContext;
#endif
	uint32_t 			SystemEffectMode;//用于标记system当前的音效模式，进入USB PHONE模式时需要使用AEC，采用和HFP音效
...
//Task & App Config
	...
	if(GetSystemMode() == AppModeUsbPhone)
	{
		DBG("@!@@!USB phone mode\n");
		//AEC
		Audio_AECInit(&UsbDevicePlayCt->AecContext);
		Audio_AECReset(&UsbDevicePlayCt->AecContext, 5);

		AudioADC1ParamsSet(16000, 5, 2);
		//DAC0/DACX
			...
			AudioDAC_Reset(DAC1);
			AudioDAC_SampleRateChange(DAC1, 16000);//16K
			...
			AudioDAC_Reset(DAC0);
			AudioDAC_SampleRateChange(DAC0, 16000);//16K
	...
	if(GetSystemMode() == AppModeUsbPhone)
	{
		UsbDevicePlayCt->AudioCoreUsb->AudioEffectProcess = (AudioCoreProcessFunc)AudioEffectProcessUsbPhone;
	}
#endif
#ifdef CFG_FUNC_RECORDER_EN
...
#ifdef CFG_FUNC_AUDIO_EFFECT_EN	
	UsbDevicePlayCt->SystemEffectMode = mainAppCt.EffectMode;
#ifdef CFG_APP_USB_PHONE_MODE_EN
	if(GetSystemMode() == AppModeUsbPhone)
	{
		...
		Audio_AECEffectInit(USB_PHONE_AEC_ECHO_LEVEL, USB_PHONE_AEC_NOISE_LEVEL);
...
bool UsbDevicePlayKill(void)
{
	...
	{
		//AudioCoreProcessConfig((void*)AudioNoAppProcess);
		DBG("USBPHone_AECDeinit11\n");
		osTaskDelay(5);
		Audio_AECDeinit(&UsbDevicePlayCt->AecContext);
		DBG("USBPHone_AECDeinit22\n");
	...
	mainAppCt.EffectMode = UsbDevicePlayCt->SystemEffectMode;
...
uint16_t UsbAudioMicDataSet(void *Buffer, uint16_t Len)
{
	...
	for(i = 0; i < temp0; i++)
	{
		...
		if((UsbAudioMic.AudioSampleRate != CFG_PARA_SAMPLE_RATE) && (GetSystemMode() == AppModeUsbDevicePlay))
		{
...
void UsbAudioTimer1msProcess(void)
{
	...
	if(UsbAudioMic.AltSet)//open stream
	{
		...
		if(GetSystemMode() == AppModeUsbPhone)
		{
			if(AudioCore.AudioSource[MIC_SOURCE_NUM].Enable == FALSE)
			{
...
{
	if(Audio_AECRingDataSpaceLenGet(&UsbDevicePlayCt->AecContext) > Len)
	{
		Audio_AECRingDataSet(&UsbDevicePlayCt->AecContext, PcmIn, Len);
	}
}

int16_t * UsbPhoneRingDataGet(uint32_t Len)
{
	return (int16_t *)Audio_AecInBuf(&UsbDevicePlayCt->AecContext, Len);
}
#endif
