...
void prinfBtConfigParams(void)
{
	...
	APP_DBG("tx power:0x%x\n", btStackConfigParams->bt_TxPowerValue);
	APP_DBG("tx power_recon:0x%x\n", btStackConfigParams->bt_TxPowerValue_Recon);
	APP_DBG("**********\n");
}
...
void LoadBtConfigurationParams(void)
{
	...
	//蓝牙公共配置参数配置如下
	//bt trim (range: 0x05~0x1f)
	if((btStackConfigParams->bt_trimValue > 0x1f)||(btStackConfigParams->bt_trimValue < 0x05))
	{
		paramsUpdate = 1;
		BT_DBG("No match: bt_trimValue\n");
		btStackConfigParams->bt_trimValue = BT_TRIM;
	}

	//bt tx power
	if(btStackConfigParams->bt_TxPowerValue > 23)
	{
		paramsUpdate = 1;
		BT_DBG("No match: bt_TxPowerValue\n");
		btStackConfigParams->bt_TxPowerValue = BT_TX_POWER_LEVEL;
	}
	if(btStackConfigParams->bt_TxPowerValue_Recon > 23)
	{
		paramsUpdate = 1;
		BT_DBG("No match: bt_TxPowerValue_Recon\n");
		btStackConfigParams->bt_TxPowerValue_Recon = BT_TX_POWER_LEVEL;
	}

	//bt pin code  此参数暂时使用SDK默认参数
...
void ConfigBtBbParams(BtBbParams *params)
{
	...
	SetRfTxPwrMaxLevel(btStackConfigParams->bt_TxPowerValue, btStackConfigParams->bt_TxPowerValue_Recon);
