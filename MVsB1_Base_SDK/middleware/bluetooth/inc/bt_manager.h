...
{
	//BT参量表中排序
	uint8_t			bt_LocalDeviceAddr[BT_ADDR_SIZE];		//6  //flash mac顺序: NAP-UAP-LAP (此变量用于保存flash的参数,不能随意修改)
	uint8_t			ble_LocalDeviceAddr[BT_ADDR_SIZE];		//12
	uint8_t			bt_LocalDeviceName[BT_NAME_SIZE];		//52
	uint8_t			ble_LocalDeviceName[BT_NAME_SIZE];		//92 //预留:现BLE name是通过BLE工具生成的广播包中包含

	uint8_t			bt_trimValue;					//93  //蓝牙频偏值
	uint8_t			bt_TxPowerValue;				//94  //正常使用时蓝牙发射功率
	uint8_t			bt_TxPowerValue_Recon;			//95  //重连时蓝牙发射功率

	uint8_t			reserve[13];					//95+13=108  //此处保留,为上位机配置蓝牙参量数据预留
	
	uint8_t			bt_simplePairingFunc;			//109
	uint8_t			bt_pinCode[16];					//125
	uint8_t			bt_pinCodeLen;					//126
